    ...
    class Solution {
        public List<Integer> findSubstring(String s, String[] words) {
            ...
            for (int i = 0; i < s.length() - allWordLength + 1; i++) {
                String substring = s.substring(i, i + allWordLength);
                Map<String, Integer> subMap = new HashMap<>();

                for (int j = 0; j < substring.length(); j += singleWordLength) {
                    String key = substring.substring(j, j + singleWordLength);
                    subMap.put(key, subMap.getOrDefault(key, 0) + 1);
                }

                if (subMap.equals(needMap)) res.add(i);
            }
