...
public class P2542MaximumSubsequenceScore {
    public static void main(String[] args) {
        Solution solution = new P2542MaximumSubsequenceScore().new Solution();
        // TO TEST
    }
    ...
    class Solution {
        public long maxScore(int[] nums1, int[] nums2, int k) {
            ...
            Arrays.sort(indexs, (i, j) -> nums2[j] - nums2[i]);

            long res = 0;
            long sum = 0;
            PriorityQueue<Integer> minHeap = new PriorityQueue<>();
            for (int i = 0; i < k - 1; i++) {
                minHeap.offer(nums1[indexs[i]]);
                sum += nums1[indexs[i]];
            }
            ...
            for (int i = k - 1; i < n; i++) {
                minHeap.offer(nums1[indexs[i]]);
                sum += nums1[indexs[i]];
                res = Math.max(res, sum * nums2[indexs[i]]);
                sum -= minHeap.poll();
            }
