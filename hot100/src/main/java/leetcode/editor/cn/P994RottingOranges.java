...
//
//
// Related Topics 广度优先搜索 数组 矩阵 👍 765 👎 0
...
public class P994RottingOranges {
    public static void main(String[] args) {
        ...
        System.out.println(solution.orangesRotting(GridUtils.generateIntGrid("[[2,1,1],[1,1,1],[0,1,2]]")));
    }
    ...
    class Solution {
        ...
        public int orangesRotting(int[][] grid) {
            Deque<int[]> rots = new ArrayDeque<>();

            for (int i = 0; i < grid.length; i++) {
                for (int j = 0; j < grid[i].length; j++) {
                    if (grid[i][j] == 2) {
                        rots.addLast(new int[]{i, j});
                    } else if (grid[i][j] == 1) {
                        fresh++;
                    }
                }
            ...
            while (!rots.isEmpty() && fresh > 0) {
                Deque<int[]> newRots = new ArrayDeque<>();

                while (!rots.isEmpty()) {
                    int[] rotIndex = rots.removeFirst();
                    int x = rotIndex[0];
                    int y = rotIndex[1];
                    for (int i = 0; i < position.length; i++) {
                        ...
                        if (nextX >= 0 && nextY >= 0 && nextX < grid.length && nextY < grid[0].length && grid[nextX][nextY] == 1) {
                            grid[nextX][nextY] = 2;
                            fresh--;
                            newRots.addLast(new int[]{nextX, nextY});
                        }
                    }
                }
